...
export default function App() {
    ...
    try {
            ...
            setMessages(prev =>
              prev.map(msg =>
                msg.id === assistantMessageId
                  ? { ...msg, content: chunk.fullText || msg.content }
                  : msg
              )
            );
      ...
      if (response.error && !streamingCompleted) {
        ...
        try {
          ...
          errorContent =
            parsedError.error?.message || parsedError.message || response.error;
        } catch {
  ...
  const handleStopResponse = async () => {
    try {
      const result = await stopAIResponse();
      if (result.success) {
        setIsStreaming(false);
        setIsLoading(false);
        toast.success('Response stopped');
      } else {
        toast.error(result.error || 'Failed to stop response');
      }
    } catch (error) {
