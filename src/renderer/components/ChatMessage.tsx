      ...
      >
        {isStreaming ? (
          <div className="flex items-center space-x-3">
            ...
            <span className="text-slate-600 text-sm font-medium">AI is thinking...</span>
          </div>
        ) : (
              ...
              <div className="flex flex-wrap gap-3">
                {message.images.map((image, index) => (
                  <div key={index} className="relative">
                    ...
                    />
                    {image.name && (
                      <div className={`text-xs mt-2 opacity-75 ${isUser ? "text-slate-100" : "text-slate-500"}`}>
                        {image.name}
                      </div>
                    )}
                  </div>
              ...
              <div className={`prose ${isUser ? "prose-invert" : "prose-slate"} max-w-none`}>
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    p: ({ children }) => <p className="mb-3 leading-relaxed text-current">{children}</p>,
                    ul: ({ children }) => (
                      <ul className="list-disc list-inside mb-3 space-y-1 text-current">{children}</ul>
                    ),
                    ol: ({ children }) => (
                      <ol className="list-decimal list-inside mb-3 space-y-1 text-current">{children}</ol>
                    ),
                        ...
                        return (
                          <code
                            className={`text-sm font-mono px-1.5 py-0.5 rounded-md ${
                              isUser ? "bg-slate-500/20 text-slate-100" : "bg-slate-200 text-slate-700"
                            }`}
                          >
                            {children}
                          </code>
                    ...
                    pre: ({ children }) => <pre className="rounded-xl overflow-x-auto mb-3">{children}</pre>,
                    blockquote: ({ children }) => (
                      <blockquote
                    ...
                    th: ({ children }) => (
                      <th
                        className={`border border-slate-200 px-4 py-3 font-semibold ${
                          isUser ? "bg-blue-500/20 text-blue-100" : "bg-slate-100 text-slate-700"
                        }`}
                      >
                        {children}
                      </th>
