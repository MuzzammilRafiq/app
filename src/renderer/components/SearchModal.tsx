...
import { LoadingSVG, SearchSVG } from "./icons";
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      if (response?.success && response.results) {
        const searchResults: SearchResult[] = response.results.filter((path: string) => path && path.trim());
        ...
        <form onSubmit={handleSubmit} className="p-4 border-b border-gray-100">
          <div className="relative">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-3 border-0 rounded-lg bg-gray-50 text-gray-900 placeholder-gray-500 focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all duration-200 text-lg"
            />
          </div>
        </form>
