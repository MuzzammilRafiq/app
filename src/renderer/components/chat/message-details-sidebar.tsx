...
export default function MessageDetailsSidebar({ isOpen, onClose, plans, logs, sources }: MessageDetailsSidebarProps) {
  const hasAny = (plans?.length ?? 0) + (logs?.length ?? 0) + (sources?.length ?? 0) > 0;
    ...
    <>
      ...
      >
        ...
        <div className="h-[calc(100%-49px)] overflow-y-auto p-3 space-y-3">
          {!hasAny && <div className="text-sm text-slate-600">No content available.</div>}

          {plans && plans.length > 0 && (
            <section>
              <div className="text-xs font-semibold text-blue-700 mb-1">Plan</div>
          ...
          {logs && logs.length > 0 && (
            <section>
              <div className="text-xs font-semibold text-amber-700 mb-1">Log</div>
              <div className="space-y-2">
                {logs.map((m) => (
                  <div key={m.id}>
          ...
          {sources && sources.length > 0 && (
            <section>
              <div className="text-xs font-semibold text-emerald-700 mb-1">Sources</div>
              <div className="space-y-2">
                {sources.map((m) => (
                  <div key={m.id}>
